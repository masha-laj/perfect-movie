...
import kotlinx.android.synthetic.main.fragment_rated.view.*
...
class RatedFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val _view = inflater.inflate(R.layout.fragment_rated, container, false)
        ...
        _view.ratedRV.layoutManager = LinearLayoutManager(activity)
        _view.ratedRV.setHasFixedSize(true)
        _view.ratedRV.adapter = adapter
    ...
    private fun render(state: AppState) {
        if (state.isRatedLoadingItems) showLoading()
        else hideLoading()

        adapter.submitList(state.ratedMovies)
    }

    private fun hideLoading() {
        activity?.runOnUiThread {
            view?.ratedPB?.visibility = View.INVISIBLE
        }
    }

    private fun showLoading() {
        activity?.runOnUiThread {
            view?.ratedPB?.visibility = View.VISIBLE
        }
    }
}